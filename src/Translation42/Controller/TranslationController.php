...
class TranslationController extends AbstractAdminController
{
    ...
    public function dashboardAction()
    {
        ...
        foreach ($textDomainConfigs as $textDomainConfig) {
            ...
            $numTranslated = $tableGateway->select(['textDomain' => $textDomainConfig['text_domain'], $whereTranslationNotNull])->count();
            $percentTranslated = (int)($numTranslated / $numTotal * 100);

            $numMissingTotal = $tableGateway->select(['textDomain' => $textDomainConfig['text_domain'], 'translation' => null])->count();
            $percentMissingTotal = (int)($numMissingTotal / $numTotal * 100);

            $numMissingManual = $tableGateway->select(
                ['textDomain' => $textDomainConfig['text_domain'], 'status' => Translation::STATUS_MANUAL, 'translation' => null]
            )->count();
            $percentMissingManual = (int)($numMissingManual / $numTotal * 100);

            $numMissingAutoGenerated = $tableGateway->select(
                ['textDomain' => $textDomainConfig['text_domain'], 'status' => Translation::STATUS_AUTO, 'translation' => null]
            )->count();
